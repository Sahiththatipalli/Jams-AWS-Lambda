...
def get_yesterday_label(fmt: Optional[str] = None, tz_name: Optional[str] = None) -> str:
    """
    Return yesterday's calendar day as a string (default %Y%m%d).
    """
    fmt = fmt or os.getenv("FILENAME_DATE_FORMAT", "%Y%m%d")
    # reuse your existing tz helper(s)/pattern — this mirrors get_today_label
    tz = tz_name or os.getenv("LOCAL_TZ")
    try:
        if tz:
            from zoneinfo import ZoneInfo  # if already imported above, omit this line
            now = datetime.datetime.now(ZoneInfo(tz))
        else:
            now = datetime.datetime.utcnow()
    except Exception:
        now = datetime.datetime.utcnow()
    return (now - datetime.timedelta(days=1)).strftime(fmt)
...
@default_retry()
def upload_files_to_box_yesterday(client, root_folder_id: str, local_dir: str, context=None):
    """
    Upload files into a single Box subfolder named with **yesterday's** date
    (e.g., '20250827') under the Box folder identified by root_folder_id.
    On name conflicts, uploads as a **new version**.
    """
    trace_id = get_or_create_trace_id(context)
    folder_name = get_yesterday_label()
    files = os.listdir(local_dir)

    logger.info(f"[{trace_id}] Uploading {len(files)} files to Box folder {root_folder_id}/{folder_name}")
    box_root = client.folder(root_folder_id)

    # Reuse your existing helper to find/create day folder
    day_folder = get_or_create_folder(client, box_root, folder_name)
