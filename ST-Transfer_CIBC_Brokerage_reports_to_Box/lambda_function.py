...
def _expand_date_tokens(s: str) -> str:
    """
    Replace {TODAY:%Y%m%d}, {YESTERDAY:%Y%m%d}, and {PREV_BIZDAY:%Y%m%d} tokens in patterns.
    Additionally, if ANCHOR_DATE is set (defaults to YESTERDAY), {TODAY:...} will be interpreted
    according to that anchor (YESTERDAY or PREV_BIZDAY).
    """
    anchor = os.getenv("ANCHOR_DATE", "YESTERDAY").upper()

    def repl(m):
        token, fmt = m.group(1), m.group(2)
        if token == "TODAY":
            if anchor == "YESTERDAY":
    ...
    return re.sub(r"\{(TODAY|YESTERDAY|PREV_BIZDAY)(?::([^}]+))?\}", repl, s)


def get_file_patterns():
    raw = os.getenv('FILE_PATTERN')
    patterns = [x.strip() for x in raw.split(',')] if raw else ['*']
    return [_expand_date_tokens(p) for p in patterns if p]
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Use a single TODAY date folder for both S3 and Box
        anchor = os.getenv("ANCHOR_DATE", "YESTERDAY").upper()
        if anchor == "YESTERDAY":
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if box_files:
                ...
                if dry_run_enabled:
                    ...
                    if os.getenv("ANCHOR_DATE", "YESTERDAY").upper() == "YESTERDAY"
                    else upload_files_to_box_prev_bizday
                    if os.getenv("ANCHOR_DATE", "YESTERDAY").upper() in ("PREV_BIZDAY", "PREVIOUS_BUSINESS_DAY", "PREV_BUSINESS_DAY")
                    else upload_files_to_box_today
                )(
                    box_client, box_folder_id, box_tmp_dir, context
                )
