...
def get_prev_business_day_label(fmt: Optional[str] = None, tz_name: Optional[str] = None) -> str:
    """
    Return previous business day as a string. Default format is YYYYMMDD.
...
def get_today_label(fmt: Optional[str] = None, tz_name: Optional[str] = None) -> str:
    """
    Return today's date string in the given format (default: %Y%m%d) using LOCAL_TZ.
    """
    fmt = fmt or os.getenv("FILENAME_DATE_FORMAT", "%Y%m%d")
    return _now_in_tz(tz_name).strftime(fmt)

# NEW: calendar yesterday label
def get_yesterday_label(fmt: Optional[str] = None, tz_name: Optional[str] = None) -> str:
    """
    Return yesterday's calendar day as a string (default YYYYMMDD).
    Uses LOCAL_TZ if available; falls back to UTC.
    """
    fmt = fmt or os.getenv("FILENAME_DATE_FORMAT", "%Y%m%d")
    tz = tz_name or os.getenv("LOCAL_TZ")
    try:
        if tz and ZoneInfo:
...
def upload_files_to_box_prev_bizday(client, root_folder_id: str, local_dir: str, context=None):
    ...
    (e.g., '20250825') under the Box folder identified by root_folder_id.
    On name conflicts, uploads as a **new version**.
    """
...
@default_retry()
def upload_files_to_box_today(client, root_folder_id: str, local_dir: str, context=None):
    """
    Upload files into a single Box subfolder named with **today's** date
    (e.g., '20250826') under the Box folder identified by root_folder_id.
    On name conflicts, uploads as a **new version**.
    """
    trace_id = get_or_create_trace_id(context)
    folder_name = get_today_label()
    files = os.listdir(local_dir)
...
@default_retry()
def upload_files_to_box_yesterday(box_client, box_parent_folder_id: str, local_dir: str, context: dict) -> None:
    """
    Upload all files in local_dir to Box into a YYYYMMDD (yesterday) folder under the given parent.
    If a file already exists, upload a new version.
    """
    trace_id = get_or_create_trace_id(context)
    date_label = get_yesterday_label()
    logger.info(f"[{trace_id}] [BOX] Uploading files to yesterday folder: {date_label}")

    parent = box_client.folder(box_parent_folder_id)
    # Ensure (or create) the day folder
    day_folder = None
    for item in parent.get_items(limit=1000):
        if item.type == "folder" and item.name == date_label:
            day_folder = box_client.folder(item.id)
            break
    if not day_folder:
        day_folder = parent.create_subfolder(date_label)
        logger.info(f"[{trace_id}] Created Box folder: {date_label}")
    ...
    for entry in os.scandir(local_dir):
        ...
        filename = entry.name
        logger.info(f"[{trace_id}] [BOX] Uploading {filename} -> {date_label}/")
        ...
        try:
            ...
            # 409 + code=item_name_in_use => upload as new version
            if e.status == 409 and getattr(e, "code", None) == "item_name_in_use":
                logger.info(f"[{trace_id}] {filename} exists, uploading as a new version.")
                items = {item.name: item for item in day_folder.get_items()}
                if filename in items:
                    items[filename].update_contents(local_path)
                    logger.info(f"[{trace_id}] Uploaded {filename} as a new version.")
                else:
                    logger.error(f"[{trace_id}] Conflict reported, but item not found in folder: {filename}")
            else:
                logger.error(f"[{trace_id}] Failed to upload {filename} to Box: {e}")
