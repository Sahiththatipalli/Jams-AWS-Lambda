...
@default_retry()
def _upload_list_to_box_folder(box_folder, files, local_dir, trace_id: str):
    """
    Upload files directly into `box_folder`. On conflict, upload as a new version.
    """
    from boxsdk.exception import BoxAPIException  # local import to match existing style
    for filename in files:
        local_path = os.path.join(local_dir, filename)
        logger.info(f"[{trace_id}] Uploading {filename} to Box folder '{box_folder.object_id}'")
        with open(local_path, 'rb') as file_stream:
            try:
                box_folder.upload_stream(file_stream, filename)
                logger.info(f"[{trace_id}] Uploaded {filename} to Box successfully")
            except BoxAPIException as e:
                if e.status == 409 and getattr(e, "code", None) == "item_name_in_use":
                    logger.info(f"[{trace_id}] {filename} exists, uploading as new version.")
                    items = {item.name: item for item in box_folder.get_items()}
                    if filename in items:
                        items[filename].update_contents(local_path)
                        logger.info(f"[{trace_id}] Uploaded {filename} as a new version in Box.")
                    else:
                        logger.error(f"[{trace_id}] Conflict reported, but file not found: {filename}")
                else:
                    logger.error(f"[{trace_id}] Failed to upload {filename} to Box: {e}")


@default_retry()
def upload_files_to_box_two_locations(client, daily_folder_id: str, archive_root_folder_id: str, local_dir: str, context=None):
    """
    Upload files to two Box locations:
      1) Daily folder: files uploaded directly (no date subfolder)
      2) Archive root: files uploaded under a subfolder named previous business day (e.g., '20250929')
    """
    trace_id = get_or_create_trace_id(context)
    files = os.listdir(local_dir)
    day_label = get_prev_business_day_label()

    logger.info(f"[{trace_id}] Two-destination Box upload: {len(files)} files")
    daily_folder = client.folder(daily_folder_id)
    archive_root = client.folder(archive_root_folder_id)
    archive_day_folder = get_or_create_folder(client, archive_root, day_label)

    # 1) Daily folder upload
    _upload_list_to_box_folder(daily_folder, files, local_dir, trace_id)

    # 2) Archive/<yyyyMMdd> upload
    _upload_list_to_box_folder(archive_day_folder, files, local_dir, trace_id)
