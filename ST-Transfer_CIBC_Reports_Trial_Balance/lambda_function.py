...
# ---------- Secondary S3 selector ----------

def is_today_trial_balance_csv(filename: str) -> bool:
    """
    True only for 'Trial_Balance_YYYYMMDD.csv' where YYYYMMDD is *today*
    (LOCAL_TZ governs 'today' via storage_utils.get_current_day_label).
    """
    today_label = get_current_day_label("%Y%m%d")
    return filename == f"Trial_Balance_{today_label}.csv"

# ----assumed role for secondary s3------

def s3_client_for_fundops(role_arn: str):
    sts = boto3.client("sts")
    creds = sts.assume_role(RoleArn=role_arn, RoleSessionName="trial-balance-upload")["Credentials"]
    return boto3.client(
        "s3",
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Secondary S3 (for today's Trial_Balance_YYYYMMDD.csv only)
        secondary_bucket = os.getenv('SECONDARY_S3_BUCKET')   # e.g., ninepoint-etl-dev
        secondary_prefix = os.getenv('SECONDARY_S3_PREFIX')   # e.g., raw/TRIAL_BALANCE

        fundops_role_arn = os.getenv('FUNDOPS_ROLE_ARN')  # already in envs
        sec_s3 = s3_client_for_fundops(fundops_role_arn) if fundops_role_arn else s3_client


        # Compute the single target folder name once (previous business day)
        date_folder = get_prev_business_day_label()
        logger.info("[%s] Using single date folder '%s' for both S3 and Box", trace_id, date_folder)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for filename in matched_files:
                try:
                    ...
                    # S3 upload (primary) - single date folder + optional KMS
                    ...
                    # ---- Secondary S3 (only today's Trial_Balance_YYYYMMDD.csv) ----
                    try:
                        if secondary_bucket and secondary_prefix:
                            if is_today_trial_balance_csv(filename):
                                sec_key = f"{secondary_prefix.rstrip('/')}/{filename}"
                                if not dry_run_enabled:
                                    _, s3_sec_duration = time_operation(
                                        sec_s3.upload_file,
                                        local_path, secondary_bucket, sec_key
                                    )
                                    log_file_transferred(trace_id, filename, "Secondary S3", s3_sec_duration)
                                else:
                                    log_dry_run_action(trace_id, f"Would upload {filename} to Secondary S3 at {sec_key}")
                            else:
                                logger.info("[%s] Secondary S3 skip: %s is not today's Trial_Balance_YYYYMMDD.csv",
                                            trace_id, filename)
                    except Exception as e:
                        warning_msg = f"Secondary S3 upload skipped due to error for {filename}: {e}"
                        warnings.append(warning_msg)
                        log_warning(trace_id, warning_msg)
            ...
            if box_files:
                ...
                if dry_run_enabled:
                    ...
                    try:
                        upload_files_to_box_prev_bizday(box_client, box_folder_id, box_tmp_dir, context)
                    except Exception as e:
