...
def get_current_day_label(fmt: Optional[str] = None, tz_name: Optional[str] = None) -> str:
    """
    Return today's calendar date string in the given format and timezone
    (default tz from LOCAL_TZ env, default format %Y%m%d).
    This is used for secondary S3 selection of Trial_Balance_YYYYMMDD.csv.
    """
    fmt = fmt or "%Y%m%d"
