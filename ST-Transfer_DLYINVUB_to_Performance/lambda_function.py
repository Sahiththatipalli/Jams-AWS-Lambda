...
def lambda_handler(event, context):
    ...
    try:
        ...
        box_folder_id_daily = os.getenv('BOX_FOLDER_ID_DAILY')
        box_folder_id_archive = os.getenv('BOX_FOLDER_ID_ARCHIVE')
        ...
        src_dir  = os.getenv('SRC_REMOTE_DIR', '.')   # <<< standard CIBC dir
        ...
        # Paths and pattern for DLYINVUB
        alt_remote_root = os.getenv("ALT_REMOTE_DIR_ROOT", "/lti/PROD")
        special_dir = f"{alt_remote_root.rstrip('/')}/{date_folder}"
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # ---- List BOTH locations ----
            try:
                # Standard dir (all NON-DLYINVUB patterns)
                logger.info("[%s] SFTP listdir_attr standard dir: %s", trace_id, src_dir)
                entries_std = src_sftp.listdir_attr(src_dir)
                all_files_std = [e.filename for e in entries_std]
                logger.info("[%s] Standard dir returned %d items", trace_id, len(all_files_std))

                # Special dir (only DLYINVUB.txt from prev biz day)
                logger.info("[%s] SFTP listdir_attr special dir: %s", trace_id, special_dir)
                entries_dly = src_sftp.listdir_attr(special_dir)
                all_files_dly = [e.filename for e in entries_dly]
                logger.info("[%s] Special dir returned %d items", trace_id, len(all_files_dly))
            except Exception as e:
            ...
            # ---- Match logic ----
            # Use FILE_PATTERN for standard dir but drop any DLYINVUB* token from it.
            std_patterns = [p for p in file_patterns if not p.lower().startswith("dlyinvub")]
            matched_std = match_files(all_files_std, include_patterns=std_patterns)

            # DLYINVUB: exact .txt, case-insensitive
            matched_dly = [f for f in all_files_dly if f.lower() == "dlyinvub.txt"]

            # Logging
            unmatched_std = set(all_files_std) - set(matched_std)
            unmatched_dly = set(all_files_dly) - set(matched_dly)
            log_matched_files(trace_id, matched_std, unmatched_std)
            log_matched_files(trace_id, matched_dly, unmatched_dly)

            # Build download plan with correct base dir per file
            download_plan = [(f, src_dir) for f in matched_std] + [(f, special_dir) for f in matched_dly]

            if not download_plan:
                warning_msg = "No files matched FILE_PATTERN (after special-case DLYINVUB routing)"
                warnings.append(warning_msg)
                log_warning(trace_id, warning_msg)

            # ---- Download & S3 ----
            downloaded_ok = []
            for filename, remote_base in download_plan:
                try:
                    remote_path = f"{remote_base.rstrip('/')}/{filename}"
            ...
            # ---- Box upload (use exactly what downloaded) ----
            box_files = downloaded_ok[:]
            log_matched_files(trace_id, box_files, set(os.listdir(tmp_dir)) - set(box_files))
            ...
            if box_files:
                ...
                warning_msg = "No files matched for Box"
                warnings.append(warning_msg)
                log_warning(trace_id, warning_msg)
