...
from storage_utils import (
    get_prev_business_day_str,   # for S3 + Box
...
def get_file_patterns():
    """
    Reads FILE_PATTERN (comma-separated). Supports the token {YYYYMMDD},
    which resolves to the previous business day in America/Toronto (configurable via TIMEZONE).
    Examples:
      FILE_PATTERN = NPP_Aladdin_Capstock_{YYYYMMDD}.csv
      FILE_PATTERN = A_{YYYYMMDD}.csv,B_{YYYYMMDD}.txt
    """
    val = os.getenv('FILE_PATTERN')
    patterns = [x.strip() for x in val.split(',')] if val else ['*']
    prev_bday = get_prev_business_day_str()
    resolved = [p.replace('{YYYYMMDD}', prev_bday) for p in patterns]
...
def download_and_upload_to_s3(
    ...
    prev_key = get_prev_business_day_str()  # YYYYMMDD
    ...
    for filename in files:
        ...
        try:
            ...
            s3_key = f"{prefix}/{prev_key}/{filename}" if prefix else f"{prev_key}/{filename}"
...
def upload_files_to_external_sftp(
    ...
    # NEW: Do NOT add date subfolders; write directly to remote_dir
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                if box_files:
                    ...
                    if DRY_RUN:
                        log_dry_run_action(
                            "Would upload files to Box (prev-business-day)",
                            box_tmp_dir,
                            box_folder_id
                        )
